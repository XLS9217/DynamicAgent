...
- **Parameters**:
    ...
    - **Types**: Generated by Pydantic from Python annotations:
        - `str` maps to `string` (also the default for unannotated parameters).
        - `int` maps to `integer`; `float` maps to `number`.
        - `bool` maps to `boolean`; `dict` maps to `object`.
        - Typed lists and dictionaries support nested annotations.
        - Unions, nullable types, and `Literal` values retain their schema constraints.
        - Pydantic models expose nested fields, defaults, constraints, and requiredness.
...
- **Execution**: Schema generation does not convert incoming dictionaries into Pydantic instances. Use `Model.model_validate(value)` inside a tool when you need an instance.
- **Signatures**: Use named parameters; positional-only parameters and variadic arguments are rejected. Unresolvable annotations raise an error instead of silently becoming strings.
