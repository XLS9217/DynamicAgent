...
async def build_workflow(
    workflow_cls,
    *args,
    workflow_log_path: str | Path | None = None,
    **kwargs
):
    load_dotenv()
    ...
    if workflow_log_path is None:
        ...
        if cache_dir:
            workflow_log_path = Path(cache_dir) / log_name
        else:
            workflow_log_path = Path.cwd() / log_name
    else:
...
class WorkflowBase(ABC):
    ...
    def _resolve_log_path(self) -> Path:
        if self._workflow_log_path is not None:
            return self._workflow_log_path
        load_dotenv()
        cache_dir = os.getenv("CACHE_DIR")
    ...
    def get_log(self) -> list[dict]:
        if self._workflow_log_path is None or not self._workflow_log_path.exists():
            return []
        logs = []
        with open(self._workflow_log_path, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
    ...
    def save_jsonl(self, file_path: str | Path):
        if self._workflow_log_path is None:
            return
        source = self._workflow_log_path
