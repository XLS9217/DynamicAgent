"""
Smoke test knowledge inbound with single-entity-type identification.

This script uses the same prompt and text as inbound_multiple.py. The only
intended behavior difference is entity_limit_one=True.
"""
import asyncio
import json
import os
...
async def main():
    ...
    try:
        ...
        await DynamicAgentClient.create_bucket(
            name=BUCKET_NAME,
            description="Smoke test bucket for one-entity inbound",
        )
        ...
        assert len(blueprints) == 1, "expected entity_limit_one to create exactly one blueprint"
